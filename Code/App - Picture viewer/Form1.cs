...
namespace Project_Phase1
{
    public partial class Form1 : Form
    {
        ...
        Image<Bgr, Byte> loaded = new Image<Bgr, Byte>("0.jpg") ;
     ...
     //   IBGFGDetector<Bgr> _forgroundDetector;
        ...
        String gesture = "Unrecognized";
        String prevgesture = "Unrecognized";
      ...
      MCvMoments mv;
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            ibPicture.Image = loaded; 
            bmcapprocess = true;
            tbImageName.Text = "0.jpg";
        }
        ...
        private void DrawAndComputeFingersNum()
        {
           ...
           /* for(int i=0; i< defects.Total; i++) 
            tbImageName.Text += "(" + defectArray[i].StartPoint.X.ToString() + " , " + defectArray[i].StartPoint.Y.ToString() + ")\n";*/
           ...
           * 
           ...
           /*     else if (fingerNum == 1 && contourAxisAngle > 130 && contourAxisAngle < 165)
                ...
                else if (fingerNum == 2 && contourAxisAngle > 130 && contourAxisAngle < 165)
                {
                    ...
                    {
                        i = (i + 1) % 5;
                        loaded = new Image<Bgr, Byte>(i.ToString() + ".jpg");
                        ibPicture.Image = loaded;
                        tbImageName.Text = i.ToString() + ".jpg";
                    }
                }

                else if (fingerNum == 2 && contourAxisAngle > 60 && contourAxisAngle < 120)
                {
                    prevgesture = gesture;
         ...
         /*       else if (fingerNum == 2)
                {
                 ...
                 /*   if (prevgesture != "Scroll")
                      {
                         startx = box.center.X;
                         starty = box.center.Y;
                      }
                      double changex = box.center.X - startx;
                      double changey = box.center.Y - starty;
      */

        /*             prevgesture = gesture;
                    tbImageName.Text = prevgesture;
                    gesture = tbGesture.Text = "Scroll";
                   
                   


                       /*   int scrollx = ibPicture.HorizontalScrollBar.Value;
                          if (scrollx > ibPicture.HorizontalScrollBar.Minimum + 5 && ibPicture.HorizontalScrollBar.Maximum > scrollx + 5)
                              ibPicture.HorizontalScrollBar.Value = scrollx + 5;
                          int scrolly = ibPicture.VerticalScrollBar.Value;
                          if (scrolly > ibPicture.VerticalScrollBar.Minimum +5 && ibPicture.VerticalScrollBar.Maximum > scrolly + 5)
                              ibPicture.HorizontalScrollBar.Value = scrollx + 5;
                        */  
     
	      /*    int newX = ibPicture.Location.X + (int)changex;
	          int newY = ibPicture.Location.Y + (int)changey;
              if (newX + ibPicture.Width < ibPicture.Width && ibPicture.Width + newX > ibPicture.Width)
                  ibPicture.Location = new Point(newX, ibPicture.Location.Y);
              if (newY + ibPicture.Height < ibPicture.Height && ibPicture.Height + newY > ibPicture.Height)
                  ibPicture.Location = new Point(ibPicture.Location.X, newY);
*/

       /*             int newX = ibPicture.Location.X + 5;
                    int newY = ibPicture.Location.Y + 5;
              //      if (newX + ibPicture.Width < ibPicture.Width && ibPicture.Width + newX > ibPicture.Width)
                        ibPicture.Location = new Point(newX, ibPicture.Location.Y);
             //       if (newY + ibPicture.Height < ibPicture.Height && ibPicture.Height + newY > ibPicture.Height)
                        ibPicture.Location = new Point(ibPicture.Location.X, newY);
            ...
            /*    else if (fingerNum == 1 && contourAxisAngle > 15 && contourAxisAngle < 30)
                    gesture = tbGesture.Text = "Thumbs Down!";*/

                else if (fingerNum == 5)
                {
                    prevgesture = gesture;
                    gesture = tbGesture.Text = "Zoom In";
                    double zoom = ibPicture.ZoomScale;
                    ibPicture.SetZoomScale(zoom + 0.005, pt);
                ...
                {
                    prevgesture = gesture;
                    gesture = tbGesture.Text = "Zoom out";
                    double zoom = ibPicture.ZoomScale;
                    ibPicture.SetZoomScale(zoom - 0.005, pt);
